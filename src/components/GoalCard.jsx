...
export default function GoalCard({ goal }) {
  ...
  const textColor = isComplete ? 'text-[#166534]' : 'text-primary';

  return (
    <div className="bg-surface-container-lowest p-lg rounded-xl custom-shadow border border-outline-variant flex flex-col gap-lg transition-transform duration-200 ease-out hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex items-center justify-between gap-md">
        <div className="flex items-center gap-md">
          <div
            className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              isComplete ? 'bg-[#DCFCE7] text-[#166534]' : 'bg-surface-container-low text-primary'
            }`}
          >
            <Icon size={20} strokeWidth={2} />
          </div>
          <div>
            <h3 className="font-body text-body font-bold text-on-surface">{name}</h3>
        ...
        {isComplete && (
          <span className="inline-flex items-center rounded-full bg-[#DCFCE7] px-sm py-[2px] text-[11px] font-semibold text-[#166534] whitespace-nowrap">
            Goal Reached! 🎉
          </span>
        )}
      </div>
